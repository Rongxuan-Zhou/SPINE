...
def execute_oracle_grasp(env, task: str, mode: str = "waypoints") -> None:
    ...
    # Waypoint approach in OSC pose space
    ...
    if mode == "teleport":
        ...
        # ensure the nut is rigidly seated in the gripper before insertion
        _teleport_object_to_gripper(env, body_id, z_min)
...
def run_episode(
    env, policy, sigma: float, horizon: int, task: str, debug: bool, oracle_mode: str
) -> dict:
    obs = env.reset()
