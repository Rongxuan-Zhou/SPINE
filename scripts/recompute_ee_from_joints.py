...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Recompute EE pose from joint_positions via robosuite (Panda)."
    )
    parser.add_argument(
        "input", type=Path, help="Input trajectory JSON with joint_positions"
    )
    parser.add_argument(
        "output", type=Path, help="Output JSON with updated end_effector_pose"
    )
    parser.add_argument(
        "--env", type=str, default="Lift", help="robosuite env name (default: Lift)"
    )
...
def main() -> None:
    ...
    for f in frames:
        ...
        if len(joints) != len(robot._ref_joint_pos_indexes):
            raise ValueError(
                f"期望 {len(robot._ref_joint_pos_indexes)} 个关节，得到 {len(joints)}"
            )
        ...
        ee_quat = robot._hand_quat.astype(
            float
        ).tolist()  # order as provided by robosuite (x,y,z,w)
